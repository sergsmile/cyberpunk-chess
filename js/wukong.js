...
var Engine = function(boardSize, lightSquare, darkSquare, selectColor) {
  ...
  */
  ...
  function randomizePieceSquareTables() {
    ...
    Object.keys(pst).forEach(phase => {
      Object.keys(pst[phase]).forEach(piece => {
        pst[phase][piece] = [...originalPst[phase][piece]];
    });
  });
  const phase = opening;
  const pieces = [PAWN_PST, KNIGHT_PST, BISHOP_PST, ROOK_PST, QUEEN_PST, KING_PST];
  for (let piece of pieces) {
    for (let square = 0; square < 128; square++) {
      if ((square & 0x88) == 0) {
        ...
        // console.log(`  Square ${engine.squareToString(square)}: ${originalValue} -> ${pst[phase][piece][square]}`);
        }
      }
    }
  ...
  function negamax(alpha, beta, depth, nullMove) {
    ...
    // check for draw conditions: threefold repetition or fifty-move rule
        ...
        for (var col = 0; col < 16; col++) {
          ...
          if ((square & 0x88) == 0) {
            ...
            if (botName === 'SPHYNX') {
              ...
              pieceImage = 'img/sphynx-set/' + (board[square]) + '.webp'; // set default images
            }
            document.getElementById(square).innerHTML = 
              '<img style="width: ' + 
               (boardSize ? boardSize / 8: 400 / 8) + 
              'px" draggable="true" src ="' + pieceImage + '">';
          }
        }
      }
    }
  ...
  return {
    ...
    // randomize piece square tables
    randomizePieceSquareTables: function() { randomizePieceSquareTables(); },
